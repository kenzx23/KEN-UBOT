...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Si Goblok Ingin Mengaktifkan Perintah Global Banned!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Si Jamet Ini`")
    ...
    if user:
        if user.id == 1805518906:
            return await dark.edit(
                f"`Anda Tidak Bisa Melakukan Global Banned Ke Ken, Dia Adalah Pembuat Saya!`"
            )
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"`Global Banned Aktif Ya Goblok✅`")
            except BaseException:
                b += 1
    else:
...
async def gunben(userbot):
    ...
    me = await dc.client.get_me()
    if not sender.id == me.id:
        dark = await dc.reply("`Membatalkan Perintah Global Banned Si Jamet Ini!!!`")
    else:
