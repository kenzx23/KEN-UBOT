...
async def pingme(pong):
    ...
    start = datetime.now()
    await pong.edit("⚡")
    await pong.edit("__**BABI🐖*__")
    await pong.edit("__**BAB🐖I**__")
    await pong.edit("__**BA🐖BI**__")
    await pong.edit("__**B🐖ABI**__")
    await pong.edit("__**🐖BABI🐖**__")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**⭐ᴋᴇɴ​ ᴘɪɴɢ⭐**\n"
                    f"⭐ **ᴘɪɴɢ:** "
                    f"`%sms` \n"
                    f"⭐ **ᴏɴʟɪɴᴇ:** "
                    f"`{uptime}` \n" % (duration))
